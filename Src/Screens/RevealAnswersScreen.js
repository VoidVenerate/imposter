...
const RevealAnswersScreen = ({ route, navigation }) => {
    ...
    const [playersAndAnswers, setPlayersAndAnswers] = useState([])
    ...
    const handleVote = async () => {
        ...
        try {
            await VoteForPlayer(gameId, playerName, selectedVote.id)
            setHasVoted(true)
            setDrawerOpen(false)
        } catch (err) {
                ...
                <View style={styles.drawerContainer}>
                    ...
                    >
                        <Text style={styles.buttonText}>
                            {selectedVote ? `🗳️  VOTE FOR ${selectedVote?.name.toUpperCase()}` : 'SELECT A PLAYER'}
                        </Text>
                    </Pressable>
                </View>
            </RightDrawer>
